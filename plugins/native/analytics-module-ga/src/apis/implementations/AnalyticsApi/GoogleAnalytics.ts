...
/**
 * Google Analytics API provider for the Glint Analytics API.
 * @public
 */
export class GoogleAnalytics implements AnalyticsApi, NewAnalyticsApi {
  ...
  /**
   * Sets the GA userId, based on the `userEntityRef` set on the glint
   * identity loaded from a given Glint Identity API instance. Because
   ...
   * Analytics User ID view in the property used to track Glint.
   */
  private async setUserFrom(identityApi: IdentityApi) {
    const { userEntityRef } = await identityApi.getGlintIdentity();
