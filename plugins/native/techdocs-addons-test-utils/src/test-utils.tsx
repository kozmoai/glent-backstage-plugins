...
import { FlatRoutes } from '@kozmoai/core-app-api';
...
import { TechDocsReaderPage, techdocsPlugin } from '@kozmoai/plugin-techdocs';
import { entityRouteRef } from '@kozmoai/plugin-catalog-react';
import { searchApiRef } from '@kozmoai/plugin-search-react';
...
export class TechDocsAddonTester {
  ...
  build() {
    ...
    techdocsStorageApi.getApiOrigin.mockResolvedValue(
      'https://glint.example.com/api/techdocs',
    );
    techdocsStorageApi.getBaseUrl.mockResolvedValue(
      `https://glint.example.com/api/techdocs/${entityName.namespace}/${entityName.kind}/${entityName.name}/${this.options.path}`,
    );
