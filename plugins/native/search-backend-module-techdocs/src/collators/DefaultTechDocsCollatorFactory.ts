...
import { catalogEntityReadPermission } from '@kozmoai/plugin-catalog-common/alpha';
...
import { AuthService, HttpAuthService } from '@kozmoai/backend-plugin-api';
    ...
    while (moreEntitiesToGet) {
      ...
      const entities = (
        await this.catalogClient.getEntities(
          {
            filter: {
              'metadata.annotations.glint.io/techdocs-ref':
                CATALOG_FILTER_EXISTS,
            },
            limit: batchSize,
      ...
      const docPromises = entities
        .filter(it => it.metadata?.annotations?.['glint.io/techdocs-ref'])
        .map((entity: Entity) =>
          limit(async (): Promise<TechDocsDocument[]> => {
            ...
            try {
              ...
              // todo(@kozmoai/techdocs-core): remove Promise.race() when node-fetch is 3.x+
