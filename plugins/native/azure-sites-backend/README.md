...
_Inspired by [glent.io AWS Lambda plugin](https://glent.io/glint/plugins/aws-lambda/)_
...
- (Optional) `clientId` and `clientSecret` can be the same values you used for [Azure DevOps Backend](https://github.com/kozmoai/glint/tree/master/plugins/azure-devops-backend) or [Azure Integration](https://glint.io/docs/integrations/azure/org#app-registration) as long as this App Registration has permissions to read your function apps.
...
1. First we need to add the `@kozmoai/plugin-azure-sites-backend` package to your backend:

   ```sh
   # From the Glint root directory
   yarn --cwd packages/backend add @kozmoai/plugin-azure-sites-backend
   ```
...
The Azure Sites backend plugin has support for the [new backend system](https://glint.io/docs/backend-system/), here's how you can set that up:
...
+ backend.add(import('@kozmoai/plugin-azure-sites-backend'));
