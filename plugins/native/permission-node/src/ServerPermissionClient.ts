 ...
 * {@link @kozmoai/plugin-permission-common#PermissionClient} that allows all
...
export class ServerPermissionClient implements PermissionsService {
  ...
  static fromConfig(
    ...
    if (
      ...
      throw new Error(
        'Service-to-service authentication must be configured before enabling permissions. Read more here https://glint.io/docs/auth/service-to-service-auth',
      );
    }
  ...
  async #shouldPermissionsBeApplied(
    ...
    let credentials: GlintCredentials;
    if (options && 'credentials' in options) {
