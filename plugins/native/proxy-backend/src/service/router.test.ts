...
import { getVoidLogger, HostDiscovery } from '@kozmoai/backend-common';
...
describe('buildMiddleware', () => {
  ...
  it('rejects malformed target URLs', async () => {
    expect(() =>
      buildMiddleware('/proxy', logger, '/test', 'glint.io'),
    ).toThrow(/Proxy target is not a valid URL/);
    expect(() =>
      buildMiddleware('/proxy', logger, '/test', { target: 'glint.io' }),
    ).toThrow(/Proxy target is not a valid URL/);
  });
});
