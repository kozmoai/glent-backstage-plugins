...
This plugin allows you to explore ADRs (Architecture Decision Records) associated with your entities, as well as discover ADRs across other entities using Glint Search. Use this to inform your own architectural decisions based on the experiences of previous projects.
...
yarn --cwd packages/app add @kozmoai/plugin-adr
```

2. Make sure the [ADR backend plugin](../adr-backend/README.md) is installed.

3. [Configure integrations](https://glint.io/docs/integrations/) for all sites you would like to pull ADRs from.
...
import { EntityAdrContent, isAdrAvailable } from '@kozmoai/plugin-adr';
...
2. Add `glint.io/adr-location` annotation to your `catalog-info.yaml`:

```yaml
metadata:
  annotations:
    glint.io/adr-location: <RELATIVE_PATH_TO_ADR_FILES_DIR>
```

The value for `glint.io/adr-location` should be a path relative to your `catalog-info.yaml` file or a absolute URL to the directory which contains your ADR markdown files.

For example, if you have the following directory structure, you would set `glint.io/adr-location: docs/adrs`:
...
First, make sure to setup Glint Search with the [ADR backend plugin](../adr-backend/README.md).
...
import { AdrSearchResultListItem } from '@kozmoai/plugin-adr';
          ...
          case 'adr':
            return (
              <AdrSearchResultListItem
                ...
                // https://glint.io/docs/features/search/how-to-guides#2-using-an-extension-in-your-glint-app
