...
import { createSchemaFromZod } from '@kozmoai/frontend-plugin-api';

const catalogTagCatalogFilter = createCatalogFilterExtension({
  name: 'tag',
  loader: async () => {
    const { EntityTagPicker } = await import('@kozmoai/plugin-catalog-react');
...
const catalogListCatalogFilter = createCatalogFilterExtension({
  ...
  loader: async ({ config }) => {
    const { UserListPicker } = await import('@kozmoai/plugin-catalog-react');
