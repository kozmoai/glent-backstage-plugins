...
import { ExtendedHttpServer } from '@kozmoai/backend-app-api';
  ...
  beforeEach(async () => {
    ...
    const { server } = await startTestBackend({
      features: [
        ...
        import('@kozmoai/plugin-kubernetes-backend/alpha'),
        import('@kozmoai/plugin-permission-backend/alpha'),
        import('@kozmoai/plugin-permission-backend-module-allow-all-policy'),
        createBackendModule({
          pluginId: 'kubernetes',
...
`;
  ...
  it('serves permission integration endpoint', async () => {
    const response = await request(app)
      .get('/api/kubernetes/.well-known/glint/permissions/metadata')
