...
import { getBearerTokenFromAuthorizationHeader } from '@kozmoai/plugin-auth-node';
import {
 ...
 * @public
 */
export const HEADER_KUBERNETES_CLUSTER: string = 'Glint-Kubernetes-Cluster';
...
export const HEADER_KUBERNETES_AUTH: string =
  'Glint-Kubernetes-Authorization';
...
export class KubernetesProxy {
      ...
      middleware = createProxyMiddleware({
        ...
        router: async req => {
          ...
          if (authHeader) {
            ...
            // Map Glint-Kubernetes-Authorization-X-X headers to a KubernetesRequestAuth object
  ...
  private static authHeadersToKubernetesRequestAuth(
    ...
    return Object.keys(originalHeaders)
      .filter(header => header.startsWith('glint-kubernetes-authorization'))
      .map(header =>
