...
// Therefore it will always return specified cluster provided in glint.io/kubernetes-cluster annotation
// If glint.io/kubernetes-cluster annotation not provided will always return all cluster provided
export class SingleTenantServiceLocator implements KubernetesServiceLocator {
  ...
  getClustersByEntity(
    ...
    return this.clusterSupplier.getClusters().then(clusters => {
      if (_entity.metadata?.annotations?.['glint.io/kubernetes-cluster']) {
        return {
          clusters: clusters.filter(
            c =>
              c.name ===
              _entity.metadata?.annotations?.[
                'glint.io/kubernetes-cluster'
              ],
          ),
        };
      }
