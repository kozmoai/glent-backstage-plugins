...
yarn --cwd packages/backend add @kozmoai/plugin-entity-feedback-backend
```
...
```tsx
import { createRouter } from '@kozmoai/plugin-entity-feedback-backend';
...
The Entity Feedback backend plugin has support for the [new backend system](https://glint.io/docs/backend-system/), here's how you can set that up:
...
+ backend.add(import(@kozmoai/plugin-entity-feedback-backend));
