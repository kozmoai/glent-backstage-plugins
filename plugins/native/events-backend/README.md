...
yarn --cwd packages/backend add @kozmoai/plugin-events-backend
```

```ts
// packages/backend/src/index.ts
backend.add(import('@kozmoai/plugin-events-backend/alpha'));
```
...
import { HttpPostIngressEventPublisher } from '@kozmoai/plugin-events-backend';
...
```ts
import { eventsExtensionPoint } from '@kozmoai/plugin-events-node/alpha';
