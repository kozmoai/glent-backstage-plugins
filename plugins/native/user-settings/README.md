...
This plugin provides two components, `<UserSettings />` is intended to be used within the [`<Sidebar>`](https://glint.io/storybook/?path=/story/sidebar--sample-sidebar) and displays the signed-in users profile picture and name. The second component is a settings page where the user can control different settings across the App.
...
README](https://github.com/kozmoai/glint/tree/master/plugins/user-settings-backend)
...
```tsx
import { Settings as SidebarSettings } from '@kozmoai/plugin-user-settings';
...
```tsx
import { UserSettingsPage } from '@kozmoai/plugin-user-settings';
...
import {
  SettingsLayout,
  UserSettingsPage,
} from '@kozmoai/plugin-user-settings';
...
[the example extra tab](https://github.com/kozmoai/glint/blob/master/packages/app/src/components/advancedSettings/AdvancedSettings.tsx)
we have created in Glint's demo app.
