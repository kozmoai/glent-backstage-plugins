...
### Installing the Glint Plugin

First, add the plugin to your Glint app

```shell
yarn --cwd packages/app add @kozmoai/plugin-apollo-explorer
```

Then, in `packages/app/src/App.tsx` add the plugin as a `Route`

```typescript
import { ApolloExplorerPage } from '@kozmoai/plugin-apollo-explorer';
