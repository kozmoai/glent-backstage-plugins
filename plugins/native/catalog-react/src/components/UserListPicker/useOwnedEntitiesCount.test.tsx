...
const mockGetGlintIdentity: jest.MockedFn<
  IdentityApi['getGlintIdentity']
> = jest.fn();

jest.mock('@kozmoai/core-plugin-api', () => {
  const actual = jest.requireActual('@kozmoai/core-plugin-api');
  return {
    ...actual,
    useApi: (ref: ApiRef<any>) => {
      ...
      if (ref === identityApiRef) {
        return {
          getGlintIdentity: mockGetGlintIdentity,
        };
      }
...
describe('useOwnedEntitiesCount', () => {
  beforeEach(() => {
    ...
    mockGetGlintIdentity.mockResolvedValue({
  ...
  it(`shouldn't invoke queryEntities when filters are loading`, async () => {
    ...
    await waitFor(() => expect(mockGetGlintIdentity).toHaveBeenCalled());
