...
- 3d87019269: The `entityRouteRef` is now a well-known route that should be imported directly from `@kozmoai/plugin-catalog-react`. It is guaranteed to be globally unique across duplicate installations of the `@kozmoai/plugin-catalog-react`, starting at this version.
...
- 2916a83b9c: Deprecated `loadIdentityOwnerRefs`, since they can now be retrieved as `ownershipEntityRefs` from `identityApi.getGlintIdentity()` instead.
- 51fbedc445: Migrated usage of deprecated `IdentityApi` methods.
- c54c0d9d10: Add useEntityPermission hook
- Updated dependencies
  - @kozmoai/plugin-permission-react@0.3.0
...
- fe2a6532ff: Add Override Components for Components in @kozmoai/plugin-catalog-react
...
- 4c0f0b2003: Removed dependency on `@kozmoai/core-app-api`.
...
- Updated dependencies
  ...
  - @kozmoai/core-app-api@0.2.0
  - @kozmoai/version-bridge@0.1.1
...
- 82fbda923e: Introduce a new `StarredEntitiesApi` that is used in the `useStarredEntities` hook.
  The `@kozmoai/plugin-catalog` installs a default implementation that is backed by the `StorageApi`, but one can also override the `starredEntitiesApiRef`.
  ...
  A migration will convert the previously starred entities to the location on the first load of Glint.
...
- ea81a1f19c: Deprecated EntityContext in favor of using `useEntity`, `EntityProvider` and the new `AsyncEntityProvider` instead. This update also brings cross-version compatibility to `@kozmoai/catalog-react`, meaning that future versions can be used in parallel with this one.
- Updated dependencies
  - @kozmoai/core-components@0.4.2
  - @kozmoai/integration@0.6.4
  - @kozmoai/core-app-api@0.1.13
...
- 7028ee1ca: Expose `getEntitySourceLocation`, `getEntityMetadataViewUrl`, and
  `getEntityMetadataEditUrl` from `@kozmoai/plugin-catalog-react`.
- Updated dependencies [75b8537ce]
- Updated dependencies [27a9b503a]
- Updated dependencies [da8cba44f]
- Updated dependencies [70bc30c5b]
- Updated dependencies [eda9dbd5f]
  - @kozmoai/core-plugin-api@0.1.2
...
- 17c497b81: The default `CatalogPage` has been reworked to be more composable and make
  ...
  [CatalogPage.tsx](https://github.com/kozmoai/glint/blob/9a4baa74509b6452d7dc054d34cf079f9997166d/plugins/catalog/src/components/CatalogPage/CatalogPage.tsx)
  ...
  Customization](https://glint.io/docs/features/software-catalog/catalog-customization)
  for instructions.
...
- 0af242b6d: Introduce new cards to `@kozmoai/plugin-catalog` that can be added to entity pages:
  ...
  - In addition, `EntityHasApisCard` to display APIs of a system is added to `@kozmoai/plugin-api-docs`.

  `@kozmoai/plugin-catalog-react` now provides an `EntityTable` to build own cards for entities.
...
- 10a0124e0: Expose `useRelatedEntities` from `@kozmoai/plugin-catalog-react` to retrieve
...
- 7fc89bae2: Display owner and system as entity page links in the tables of the `api-docs`
  ...
  Move `isOwnerOf` and `getEntityRelations` from `@kozmoai/plugin-catalog` to
  `@kozmoai/plugin-catalog-react` and export it from there to use it by other
  plugins.
