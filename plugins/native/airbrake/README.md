...
The Airbrake plugin provides connectivity between Glint and Airbrake (https://airbrake.io/).
...
1. Install the Frontend plugin:
   ...
   yarn --cwd packages/app add @kozmoai/plugin-airbrake
   ```
...
2. Install the Backend plugin:
   ...
   ```bash
   # From your Glint root directory
   yarn --cwd packages/backend add @kozmoai/plugin-airbrake-backend
   ```
...
4. Create `packages/backend/src/plugins/airbrake.ts` with these contents:
   ...
   } from '@kozmoai/plugin-airbrake-backend';
...
The Airbrake backend plugin has support for the [new backend system](https://glint.io/docs/backend-system/), here's how you can set that up:
...
+ backend.add(import('@kozmoai/plugin-airbrake-backend'));
