...
import { ForwardedError } from '@kozmoai/errors';
import {
  EntityProvider,
  EntityProviderConnection,
} from '@kozmoai/plugin-catalog-node';
import { merge, isErrorResult } from 'openapi-merge';
import { getOpenApiSpecRoute } from '@kozmoai/backend-openapi-utils';
import type {
  OpenAPIObject,
  OperationObject,
  PathItemObject,
...
export class InternalOpenApiDocumentationProvider implements EntityProvider {
  ...
  async refresh(logger: LoggerService) {
    const pluginsToMerge = this.config.getStringArray(
      'catalog.providers.glintOpenapi.plugins',
    );
    ...
    const documentationEntity: ApiEntity = {
      ...
      metadata: {
        ...
        title: 'Your Glint Instance documentation',
        annotations: {
          [ANNOTATION_LOCATION]:
            'internal-package:@kozmoai/plugin-catalog-backend-module-glint-openapi',
          [ANNOTATION_ORIGIN_LOCATION]:
            'internal-package:@kozmoai/plugin-catalog-backend-module-glint-openapi',
        },
      },
