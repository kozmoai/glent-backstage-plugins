...
yarn --cwd packages/app add @kozmoai/plugin-home
```
...
```tsx
import { HomepageCompositionRoot } from '@kozmoai/plugin-home';
...
```tsx
import { createCardExtension } from '@kozmoai/plugin-home-react';
...
import { RandomJokeHomePageComponent } from '@kozmoai/plugin-home';
...
import {
  ...
  CustomHomepageGrid,
} from '@kozmoai/plugin-home';
import { Content, Header, Page } from '@kozmoai/core-components';
import { HomePageSearchBar } from '@kozmoai/plugin-search';
import { HomePageCalendar } from '@kozmoai/plugin-gcalendar';
...
```ts
// packages/app/src/App.tsx
import { VisitListener } from '@kozmoai/plugin-home';
// ...
...
In order to validate the config you can use `glint/cli config:check`
...
We are hoping that we together can build up a collection of Homepage templates. We therefore put together a place where we can collect all the templates for the Home Plugin in the [storybook](https://glint.io/storybook/?path=/story/plugins-home-templates).
