...
import { Form } from '@kozmoai/plugin-scaffolder-react/alpha';
...
describe('RepoUrlPicker', () => {
  ...
  describe('happy path rendering', () => {
    it('should render the repo url picker with minimal props', async () => {
      ...
      fireEvent.change(ownerInput, { target: { value: 'glint' } });
      ...
      expect(onSubmit).toHaveBeenCalledWith(
        expect.objectContaining({
          formData: 'github.com?owner=glint&repo=repo123',
        }),
        expect.anything(),
      );
    });
  ...
  describe('requestUserCredentials', () => {
    it('should call the scmAuthApi with the correct params', async () => {
      ...
      expect(mockScmAuthApi.getCredentials).toHaveBeenCalledWith({
        url: 'https://github.com/kozmoai/repo123',
        additionalScope: {
          repoWrite: true,
          customScopes: {
            github: ['workflow'],
    ...
    it('should call the scmAuthApi with the correct params if workspace is nested', async () => {
      ...
      expect(mockScmAuthApi.getCredentials).toHaveBeenCalledWith({
        url: 'https://gitlab.example.com/glint/mysubgroup/repo123',
        additionalScope: {
          repoWrite: true,
        },
      });
    ...
    it('should call the scmAuthApi with the correct params if only a project is set', async () => {
      ...
      await act(async () => {
        fireEvent.change(projectInput, { target: { value: 'glint' } });
        fireEvent.change(repoInput, { target: { value: 'repo123' } });
      ...
      expect(mockScmAuthApi.getCredentials).toHaveBeenCalledWith({
        url: 'https://server.bitbucket.org/glint/repo123',
        additionalScope: {
          repoWrite: true,
        },
      });
