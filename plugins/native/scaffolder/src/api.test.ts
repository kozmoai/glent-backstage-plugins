...
import { MockFetchApi, setupRequestMockHandlers } from '@kozmoai/test-utils';
...
describe('api', () => {
  ...
  const mockBaseUrl = 'http://glint/api';
  ...
  beforeEach(() => {
    ...
    identityApi.getGlintIdentity.mockReturnValue({});
  });
    ...
    describe('eventsource', () => {
      it('should work', async () => {
        ...
        expect(MockedEventSource).toHaveBeenCalledWith(
          'http://glint/api/v2/tasks/a-random-task-id/eventstream',
          {
  ...
  describe('listTasks', () => {
    ...
    it('should list task using the current user as owner', async () => {
      ...
      identityApi.getGlintIdentity.mockResolvedValueOnce({
        userEntityRef: 'user:default/foo',
      });

      const result = await apiClient.listTasks({ filterByOwnership: 'owned' });
      expect(identityApi.getGlintIdentity).toHaveBeenCalled();
