...
- Updated dependencies
  ...
  - @kozmoai/plugin-permission-react@0.4.0
...
- 1c2755991d: - **BREAKING**: Removed the `FavouriteTemplate` export in favor of the `FavoriteEntity` from `@kozmoai/plugin-catalog-react`. Please migrate any usages to that component instead if you are creating your own `TemplateCard` page.
...
- 50e0242ac2: - Moved the `JSONSchema` type from `@kozmoai/catalog-model` to `JSONSchema7`.
...
- 2f2543592c: You can now hide sections or fields in your templates based on a feature flag. For example, take this template:
    ...
    steps: [
      {
        ...
        schema: {
          title: 'Fill in some steps',
          'glint:featureFlag': 'experimental-feature',
          properties: {
            name: {
              title: 'Name',
              type: 'string',
              'glint:featureFlag': 'should-show-some-stuff-first-option',
            },
            description: {
              title: 'Description',
              type: 'string',
  ...
  If you have a feature flag that is called `experimental-feature` then your first step would be shown if you that feature flag was not active then it wouldn't be shown. The same goes for the properties in the schema. Make sure to use the key `glint:featureFlag` in your templates if you want to use this functionality.
...
- ba59832aed: Permission the Register Existing Component button
- Updated dependencies
  - @kozmoai/core-components@0.8.7-next.1
  - @kozmoai/plugin-catalog-react@0.6.13-next.1
...
- aecfe4f403: Make `ScaffolderClient` use the `FetchApi`. You now need to pass in an instance
  ...
  ```diff
  +import { fetchApiRef } from '@kozmoai/core-plugin-api';
...
- b646a73fe0: In @kozmoai/plugin-scaffolder - When user will have one option available in hostUrl or owner - autoselect and select component should be readonly.

  in @kozmoai/core-components - Select component has extended API with few more props: native : boolean, disabled: boolean. native - if set to true - Select component will use native browser select picker (not rendered by Material UI lib ).
...
- b291c3176e: Switch to using `LogViewer` component from `@kozmoai/core-components` to display scaffolder logs.
- Updated dependencies
...
- Updated dependencies
  ...
  - @kozmoai/integration-react@0.1.13
...
- Updated dependencies [8e919a6f8]
  ...
  - @kozmoai/core@0.7.12
  - @kozmoai/plugin-catalog-react@0.2.1
...
- a360f9478: Expose the catalog-import route as an external route from the scaffolder.
  ...
  when you for example are running glint with `catalog.readonly=true`.
...
- 3385b374b: Use `scmIntegrationsApiRef` from the new `@kozmoai/integration-react`.
...
- a5f42cf66: The Scaffolder and Catalog plugins have been migrated to partially require use of the [new composability API](https://glint.io/docs/plugins/composability). The Scaffolder used to register its pages using the deprecated route registration plugin API, but those registrations have been removed. This means you now need to add the Scaffolder plugin page to the app directly.
