 ...
 */
import { Entity, DEFAULT_NAMESPACE } from '@kozmoai/catalog-model';
import {
  getStaleFiles,
...
describe('lowerCaseEntityTriplet', () => {
  it('returns lower-cased entity triplet path', () => {
    const originalPath = 'default/Component/glint/index.html';
    const actualPath = lowerCaseEntityTriplet(originalPath);
    expect(actualPath).toBe('default/component/glint/index.html');
  });
});

describe('lowerCaseEntityTripletInStoragePath', () => {
  it('does not lowercase beyond the triplet', () => {
    const originalPath = 'default/Component/glint/assets/IMAGE.png';
    const actualPath = lowerCaseEntityTripletInStoragePath(originalPath);
    expect(actualPath).toBe('default/component/glint/assets/IMAGE.png');
  });
...
describe('normalizeExternalStorageRootPath', () => {
  ...
  it('returns normalized path from path with leading and trailing sep', () => {
    const originalPath = '/glint-data/techdocs/';
    const normalPath = normalizeExternalStorageRootPath(originalPath);
    expect(normalPath).toBe('glint-data/techdocs');
  });
...
describe('getStaleFiles', () => {
  const defaultFiles = [
    'default/Component/glint/index.html',
    'default/Component/glint/techdocs_metadata.json',
    'default/Component/glint/assests/javascripts/bundle.7f4f3c92.min.js',
    'default/Component/glint/assets/stylesheets/main.fe0cca5b.min.css',
  ];
  ...
  it('should not return directories as stale files if they are parent directories of new files', () => {
    const oldFiles = [...defaultFiles, 'default/Component/glint/foo'];
    const newFiles = [
      ...defaultFiles,
      'default/Component/glint/foo/bar/index.html',
    ];
    const staleFiles = getStaleFiles(newFiles, oldFiles);
    expect(staleFiles).toHaveLength(0);
  });
...
describe('getCloudPathForLocalPath', () => {
  ...
  it('should compose a remote filename including entity information', () => {
    ...
    expect(remoteBucket).toBe(`custom/component/glint/${localPath}`);
  });
  ...
  it('should use the default namespace when it is undefined', () => {
    ...
    expect(remoteBucket).toBe(
      `${DEFAULT_NAMESPACE}/component/glint/${localPath}`,
    );
  });
  ...
  it('should prepend root directory to destination', () => {
    const localPath = 'index/html';
    const rootPath = 'glint-data/techdocs/';
    const remoteBucket = getCloudPathForLocalPath(
      entity,
      localPath,
      false,
    ...
    expect(remoteBucket).toBe(
      `glint-data/techdocs/custom/component/glint/${localPath}`,
    );
  });
