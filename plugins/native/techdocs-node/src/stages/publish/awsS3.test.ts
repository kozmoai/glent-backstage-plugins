...
} from '@kozmoai/integration-aws-node';
...
describe('AwsS3Publish', () => {
  ...
  describe('publish', () => {
    it('should publish a directory', async () => {
      ...
      expect(await publisher.publish({ entity, directory })).toMatchObject({
        objects: expect.arrayContaining([
          'default/component/glint/404.html',
          `default/component/glint/index.html`,
          `default/component/glint/assets/main.css`,
        ]),
      });
    });
    ...
    it('should publish a directory when root path is specified', async () => {
      const publisher = await createPublisherFromConfig({
        bucketRootPath: 'glint-data/techdocs',
      });
      expect(await publisher.publish({ entity, directory })).toMatchObject({
        objects: expect.arrayContaining([
          'glint-data/techdocs/default/component/glint/404.html',
          `glint-data/techdocs/default/component/glint/index.html`,
          `glint-data/techdocs/default/component/glint/assets/main.css`,
        ]),
      });
    });
