 ...
 */
import { Entity, CompoundEntityRef } from '@kozmoai/catalog-model';
import { Config } from '@kozmoai/config';
import { assertError, ForwardedError } from '@kozmoai/errors';
import {
...
export class AwsS3Publish implements PublisherBase {
  ...
  static async fromConfig(
    ...
    const storageClient = new S3Client({
      customUserAgent: 'glint-aws-techdocs-s3-publisher',
  ...
  private static async buildCredentials(
    ...
    if (roleArn) {
      return fromTemporaryCredentials({
        ...
        params: {
          RoleSessionName: 'glint-aws-techdocs-s3-publisher',
          RoleArn: roleArn,
        },
        clientConfig: { region },
      });
    ...
    try {
      ...
      this.logger.error(
        `Could not retrieve metadata about the AWS S3 bucket ${this.bucketName}. ` +
          ...
          'by using environment variables. Refer to https://glint.io/docs/features/techdocs/using-cloud-storage',
      );
