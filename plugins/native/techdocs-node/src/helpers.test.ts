...
import { Entity, getEntitySourceLocation } from '@kozmoai/catalog-model';
...
jest.mock('@kozmoai/catalog-model', () => ({
  ...jest.requireActual('@kozmoai/catalog-model'),
...
const goodAnnotation = {
  annotations: {
    'glint.io/techdocs-ref':
      'url:https://github.com/kozmoai/glint/blob/master/subfolder/',
  },
};
...
const badAnnotation = {
  annotations: {
    'glint.io/techdocs-ref': 'bad-annotation',
  },
};
...
describe('parseReferenceAnnotation', () => {
  it('should parse annotation', () => {
    ...
    expect(parsedLocationAnnotation.target).toBe(
      'https://github.com/kozmoai/glint/blob/master/subfolder/',
    );
  });

  it('should throw error without annotation', () => {
    expect(() => {
      parseReferenceAnnotation('glint.io/techdocs-ref', entityBase);
    ...
    ({ techdocsRef, target }) => {
      ...
      const entity = {
        ...
        metadata: {
          name: 'test',
          annotations: {
            'glint.io/techdocs-ref': techdocsRef,
          },
        },
      };
      ...
      const result = transformDirLocation(
        entity,
        parseReferenceAnnotation('glint.io/techdocs-ref', entity),
        scmIntegrations,
      );
