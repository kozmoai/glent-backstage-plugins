...
1.  Add the OpenCost dependency to the `packages/app/package.json`:
    ...
    yarn --cwd packages/app add @kozmoai/plugin-opencost
    ```
2.  Add the `OpenCostPage` to your `packages/app/src/App.tsx`:

    ```tsx
    import { OpenCostPage } from '@kozmoai/plugin-opencost';
    ```
...
- Use the Glint proxy to communicate with the OpenCost API if necessary for authentication
- Convert AllocationReport.js to use the [Glint Table](https://glint.io/storybook/?path=/story/data-display-table--default-table)
