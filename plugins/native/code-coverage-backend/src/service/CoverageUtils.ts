...
import { ScmIntegration, ScmIntegrations } from '@kozmoai/integration';
...
export class CoverageUtils {
  ...
  async processCoveragePayload(
    ...
    const enforceScmFiles =
      entity.metadata.annotations?.['glint.io/code-coverage'] ===
        'scm-only' || false;
    ...
    if (enforceScmFiles) {
      ...
      if (!sourceLocation) {
        throw new InputError(
          `No "glint.io/source-location" annotation on entity ${stringifyEntityRef(
            entity,
          )}`,
        );
      }
