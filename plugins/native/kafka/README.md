...
yarn --cwd packages/app add @kozmoai/plugin-kafka
yarn --cwd packages/backend add @kozmoai/plugin-kafka-backend
```
...
```js
import { createRouter } from '@kozmoai/plugin-kafka-backend';
...
import { EntityKafkaContent } from '@kozmoai/plugin-kafka';
...
4. Add broker configs for the backend in your `app-config.yaml` (see
   [kafka-backend](https://github.com/kozmoai/glint/blob/master/plugins/kafka-backend/README.md)
   for more options):
