...
import { NotAllowedError } from '@kozmoai/errors';
import { AuthorizeResult } from '@kozmoai/plugin-permission-common';
import { createConditionTransformer } from '@kozmoai/plugin-permission-node';
...
import { EntityFilter } from '@kozmoai/plugin-catalog-node';
import { mockCredentials } from '@kozmoai/backend-test-utils';
...
describe('AuthorizedEntitiesCatalog', () => {
  ...
  describe('entityAncestry', () => {
    it('throws error if denied access to root entity', async () => {
      ...
      await expect(() =>
        catalog.entityAncestry('glint:default/component', {
          credentials: mockCredentials.none(),
        }),
      ).rejects.toThrow(NotAllowedError);
    });
