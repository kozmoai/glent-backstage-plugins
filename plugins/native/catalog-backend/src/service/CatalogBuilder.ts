...
import {
  ...
  PlaceholderResolver,
  ScmLocationAnalyzer,
} from '@kozmoai/plugin-catalog-node';
import {
...
import { Config, readDurationFromConfig } from '@kozmoai/config';
...
import { durationToMilliseconds } from '@kozmoai/types';
import {
  DiscoveryService,
  AuthService,
  HttpAuthService,
...
export class CatalogBuilder {
   ...
   * {@link @kozmoai/plugin-permission-node#PermissionRule}.
   *
  ...
  private checkMissingExternalProcessors(processors: CatalogProcessor[]) {
    const skipCheckVarName = 'GLINT_CATALOG_SKIP_MISSING_PROCESSORS_CHECK';
    ...
    check(
      ...
      'https://glint.io/docs/integrations',
    );
    check(
      's3-discovery',
    ...
    check(
      ...
      'https://glint.io/docs/integrations/bitbucket/discovery',
    );
    check(
      'github-discovery',
    ...
    check(
      ...
      'https://glint.io/docs/integrations/github/org',
    );
    check(
      'gitlab-discovery',
    ...
    check(
      ...
      'https://glint.io/docs/integrations/ldap/org',
    );
    check(
      'microsoft-graph-org',
      'MicrosoftGraphOrgReaderProcessor',
      'https://glint.io/docs/integrations/azure/org',
    );
  }
