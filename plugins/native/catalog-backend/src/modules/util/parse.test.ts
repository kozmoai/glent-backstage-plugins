...
import { processingResult } from '@kozmoai/plugin-catalog-node';
...
---
    ...
    expect(results[1]).toEqual({
      ...
      error: expect.objectContaining({
        message: expect.stringMatching(
          /YAML error at my-loc-type:my-loc-target, YAMLParseError: Nested mappings are not allowed in compact mappings at line 9, column 19:\s+apiVersion: glint.io\/v1alpha1/,
        ),
      }),
    });
  });
