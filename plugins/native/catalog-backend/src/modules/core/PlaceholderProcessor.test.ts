...
describe('PlaceholderProcessor', () => {
  ...
  it('works with the text resolver', async () => {
    ...
    await expect(
      processor.preProcessEntity(
        ...
        },
        {
          type: 'url',
          target:
            'https://github.com/kozmoai/glint/a/b/catalog-info.yaml',
        },
        () => {},
      ),
    ).resolves.toEqual({
    ...
    expect(reader.readUrl).toHaveBeenCalledWith(
      'https://github.com/kozmoai/glint/a/file.txt',
    );
  });
  ...
  it('works with the json resolver', async () => {
    ...
    expect(reader.readUrl).toHaveBeenCalledWith(
      'https://github.com/kozmoai/glint/a/b/file.json',
    );
  });
  ...
  it('works with the yaml resolver', async () => {
    ...
    expect(reader.readUrl).toHaveBeenCalledWith(
      'https://github.com/kozmoai/glint/a/file.yaml',
    );
  });
  ...
  it('resolves absolute path for absolute location', async () => {
      ...
      processor.preProcessEntity(
        {
          ...
          spec: {
            data: {
              $text: 'https://github.com/kozmoai/glint/catalog-info.yaml',
            },
          },
        },
        {
    ...
    expect(reader.readUrl).toHaveBeenCalledWith(
      'https://github.com/kozmoai/glint/catalog-info.yaml',
    );
  });
...
describe('yamlPlaceholderResolver', () => {
  ...
  const params: PlaceholderResolverParams = {
    key: 'a',
    value: './file.yaml',
    baseUrl: 'https://github.com/kozmoai/glint/a/b/catalog-info.yaml',
    read,
