...
import { AuthenticationError, ResponseError } from '@kozmoai/errors';
  ...
  async acceptAlert(alert: Alert, userName: string): Promise<Alert> {
    const init = {
      ...
      body: JSON.stringify({
        ...
        summary: `from ${userName} via Glint plugin`,
        eventType: 'ACCEPT',
      }),
    };
  ...
  async addImmediateMaintenance(
    ...
    const init = {
      ...
      body: JSON.stringify({
        ...
        description: `Immediate maintenance window for ${minutes} minutes. Glint — iLert plugin.`,
        createdBy: 'Glint',
