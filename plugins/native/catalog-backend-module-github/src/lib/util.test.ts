...
describe('satisfiesTopicFilter', () => {
  ...
  it('handles cases where include filter is configured', () => {
    const filter: GithubTopicFilters = {
      include: ['glint-include', 'open-source'],
      exclude: undefined,
    };
    ...
    expect(satisfiesTopicFilter(['glint-include'], filter)).toEqual(true);
    expect(
      satisfiesTopicFilter(['glint-include', 'open-source'], filter),
    ).toEqual(true);
  });
  ...
  it('handles cases where exclude filter is configured', () => {
    const filter: GithubTopicFilters = {
      include: undefined,
      exclude: ['glint-exclude', 'experiment'],
    };
    expect(satisfiesTopicFilter([], filter)).toEqual(true);
    expect(satisfiesTopicFilter(['blah', 'bleh'], filter)).toEqual(true);
    expect(
      satisfiesTopicFilter(['glint-include', 'glint-exclude'], filter),
    ).toEqual(false);
    expect(
      satisfiesTopicFilter(['experiment', 'glint-include'], filter),
    ).toEqual(false);
  });

  it('handles cases where both exclusion filters are configured', () => {
    const filter: GithubTopicFilters = {
      include: ['glint-include', 'blah'],
      exclude: ['glint-exclude', 'bleh'],
    };
    expect(satisfiesTopicFilter([], filter)).toEqual(false);
    expect(satisfiesTopicFilter(['glint-include'], filter)).toEqual(true);
    expect(satisfiesTopicFilter(['blah', 'nothing'], filter)).toEqual(true);
    expect(satisfiesTopicFilter(['glint-exclude'], filter)).toEqual(false);
