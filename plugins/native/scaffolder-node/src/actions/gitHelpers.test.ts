...
import { Git, getVoidLogger } from '@kozmoai/backend-common';
import {
  commitAndPushRepo,
  initRepoAndPush,
  commitAndPushBranch,
...
jest.mock('@kozmoai/backend-common', () => ({
  Git: {
  ...
  getVoidLogger: jest.requireActual('@kozmoai/backend-common').getVoidLogger,
}));
jest.mock('fs-extra', () => ({
  cpSync: jest.fn(),
}));
