...
This plugin provides the UI for the `@kozmoai/tech-insights-backend` plugin, in order to display results of the checks running following the rules and the logic defined in the `@kozmoai/tech-insights-backend` plugin itself.
...
yarn --cwd packages/app add @kozmoai/plugin-tech-insights
```
...
import { EntityTechInsightsScorecardContent } from '@kozmoai/plugin-tech-insights';
...
import { EntityTechInsightsScorecardCard } from '@kozmoai/plugin-tech-insights';
...
If you follow the [Backend Example](https://github.com/kozmoai/glint/tree/master/plugins/tech-insights-backend#backend-example), once the needed facts have been generated the default boolean scorecard will look like this:
