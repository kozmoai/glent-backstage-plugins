...
You need to setup the [Azure DevOps backend plugin](https://github.com/kozmoai/glint/tree/master/plugins/azure-devops-backend) before you move forward with any of these steps if you haven't already
...
**Note:** To save you time, effort, and confusion setting up these annotations manually you can use the `AzureDevOpsAnnotatorProcessor` processor which will add the `dev.azure.com/host-org` and `dev.azure.com/project-repo` annotations for you with the correct values. The Azure DevOps backend plugin has details on how to [add this processor](https://github.com/kozmoai/glint/tree/master/plugins/azure-devops-backend#processor).
...
1. First we need to add the `@kozmoai/plugin-azure-devops` package to your frontend app:
   ...
   yarn --cwd packages/app add @kozmoai/plugin-azure-devops
   ```
...
```bash
# From your Glint root directory
yarn add --cwd packages/backend @kozmoai/plugin-azure-devops-common
```
...
+  azureDevOpsPullRequestDashboardReadPermission } from '@kozmoai/plugin-azure-devops-common';
+ import {
+  AuthorizeResult,
+  PolicyDecision,
+  isPermission,
...
+ } from '@kozmoai/plugin-catalog-backend/alpha';
...
async handle(
  request: PolicyQuery,
