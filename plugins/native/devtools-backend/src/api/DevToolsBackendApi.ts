...
import { Config, ConfigReader } from '@kozmoai/config';
import { loadConfigSchema } from '@kozmoai/config-loader';
import {
  ConfigInfo,
  DevToolsInfo,
  Endpoint,
...
import { findPaths } from '@kozmoai/cli-common';
...
import { assertError } from '@kozmoai/errors';
...
export class DevToolsBackendApi {
  ...
  public async listInfo(): Promise<DevToolsInfo> {
    ...
    /* eslint-disable-next-line no-restricted-syntax */
    const paths = findPaths(__dirname);
    const glintFile = paths.resolveTargetRoot('glint.json');
    let glintJson = undefined;
    if (fs.existsSync(glintFile)) {
      const buffer = await fs.readFile(glintFile);
      glintJson = JSON.parse(buffer.toString());
    }

    const lockfilePath = paths.resolveTargetRoot('yarn.lock');
    const lockfile = await Lockfile.load(lockfilePath);

    const prefixes = ['@glint', '@internal'].concat(
    ...
    const info: DevToolsInfo = {
      ...
      glintVersion:
        glintJson && glintJson.version ? glintJson.version : 'N/A',
