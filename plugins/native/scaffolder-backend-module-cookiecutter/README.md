...
```bash
# From your Glint root directory
yarn --cwd packages/backend add @kozmoai/plugin-scaffolder-backend-module-cookiecutter
```
...
You can also visit the `/create/actions` route in your Glint application to find out more about the parameters this action accepts when it's installed to configure how you like.
...
If you are running Glint from a Docker container and you want to avoid calling a container inside a container, you can set up `cookiecutter` in your own image, this will use the local installation instead.
