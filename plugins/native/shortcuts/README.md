...
The shortcuts plugin allows a user to have easy access to pages within a Glint app by storing them as "shortcuts" in the Sidebar.
...
yarn --cwd packages/app add @kozmoai/plugin-shortcuts
```
...
```ts
// ...
export { shortcutsPlugin } from '@kozmoai/plugin-shortcuts';
```
...
import { Shortcuts } from '@kozmoai/plugin-shortcuts';
...
```ts
import { shortcutsApiRef } from '@kozmoai/plugin-shortcuts';
...
```diff
+ import { shortcutsPlugin } from '@kozmoai/plugin-shortcuts
