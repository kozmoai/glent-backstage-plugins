...
Welcome to the DevTools plugin! This plugin helps elevate useful information about a Glint instance through the frontend, which can be helpful for an integrator for troubleshooting, reviewing, and understanding their installation.
...
Lists helpful information about your current running Glint instance such as: OS, NodeJS version, Glint version, and package versions.

![Example of Info tab](./docs/devtools-info-tab.png)

#### Glint Version Reporting

The Glint Version that is reported requires `glint.json` to be present at the root of the running glint instance.
You may need to modify your Dockerfile to ensure `glint.json` is copied into the `WORKDIR` of your image.
...
# Then copy the rest of the backend bundle, along with any other files we might want (including glint.json).
COPY --chown=node:node ... glint.json ./
```

### Config

Lists the configuration being used by your current running Glint instance.

**Note:** The Config tab uses the configuration schema [defined by each plugin](https://glint.io/docs/conf/defining) to be able to mask secrets. It does this by checking that the [visibility](https://glint.io/docs/conf/defining#visibility) has been marked as `secret`. If this is not set then the secret will appear in clear text. To mitigate this it is highly recommended that you enable the [permission framework](https://glint.io/docs/permissions/overview) and [apply the proper permissions](#permissions)). If you do see secrets in clear text please contact the plugin's author to get the visibility set to secret for the applicable property.
...
Lists the status of configured External Dependencies based on your current running Glint instance's ability to reach them.

![Example of external dependencies tab](./docs/devtools-external-dependencies.png)

### Catalog Unprocessed Entities

The [Catalog Unprocessed Entities plugin](https://github.com/kozmoai/glint/tree/master/plugins/catalog-unprocessed-entities) has an optional tab that you can also be added that will show unprocessed entities:
...
1. First we need to add the `@kozmoai/plugin-devtools` package to your frontend app:

   ```sh
   # From your Glint root directory
   yarn --cwd packages/app add @kozmoai/plugin-devtools
   ```
...
3. Then after all the import statements add the following line:
   ...
   ```ts
   import { DevToolsPage } from '@kozmoai/plugin-devtools';
   ```
...
3. In the `CustomDevToolsPage.tsx` file add the following content:
   ...
   import {
     ...
     InfoContent,
   } from '@kozmoai/plugin-devtools';
   import { DevToolsLayout } from '@kozmoai/plugin-devtools';
...
1. Install and setup the [Catalog Unprocessed Entities plugin](https://github.com/kozmoai/glint/tree/master/plugins/catalog-unprocessed-entities) as per its documentation
2. Add the following import to your `CustomDevToolsPage.tsx`:

   `import { UnprocessedEntitiesContent } from '@kozmoai/plugin-catalog-unprocessed-entities';`
...
The DevTools plugin supports the [permissions framework](https://glint.io/docs/permissions/overview), the following sections outline how you can use them with the assumption that you have the permissions framework setup and working.
...
1. First we need to add the `@kozmoai/plugin-devtools-common` package to your frontend app:

   ```sh
   # From your Glint root directory
   yarn --cwd packages/app add @kozmoai/plugin-devtools-common
   ```
...
3. Then add these imports after all the existing import statements:
   ...
   ```ts
   import { devToolsAdministerPermission } from '@kozmoai/plugin-devtools-common';
...
1. First we need to add the `@kozmoai/plugin-devtools-common` package to your frontend app (skip this step if you've already done this):
...
To use this policy you'll need to make sure to add the `@kozmoai/plugin-devtools-common` package to your backend you can do that by running this command:

```sh
# From your Glint root directory
yarn --cwd packages/backend add @kozmoai/plugin-devtools-common
```
...
} from '@kozmoai/plugin-devtools-common';
```

**Note:** The group "group:default/glint-admins" is simply an example and does not exist. You can point this to any group you have in your catalog instead.
...
By default, only packages with names starting with `@glint` and `@internal` will be listed on the main "Info" tab. If you would like additional packages to be listed, you can specify the package prefixes (not regular expressions) in your `app-config.yaml`. For example, to not only provide version information about glint plugins provided by the core application (`@kozmoai/*` modules) but also `@kozmoai` and `@spotify` plugins, you can specify this configuration:
...
If you are using the `ping` type you must ensure that `ping` is available in the Host OS that is running Glint.
For example you may need to add `ping` into the Dockerfile that builds your Glint image:
