...
import { getVoidLogger, DatabaseManager } from '@kozmoai/backend-common';
...
describe('StorageTaskBroker', () => {
  ...
  const fakeSecrets = { glintToken: 'secret' } as TaskSecrets;
  ...
  it('should handle checkpoints in task state', async () => {
    ...
    await taskA.updateCheckpoint?.({
      key: 'repo.create',
      status: 'success',
      value: 'https://github.com/kozmoai/glint.git',
    });
