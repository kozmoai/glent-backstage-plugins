...
jest.mock('@kozmoai/plugin-scaffolder-node', () => {
  const actual = jest.requireActual('@kozmoai/plugin-scaffolder-node');
...
import { fetchFile } from '@kozmoai/plugin-scaffolder-node';
  ...
  it('should fetch plain', async () => {
    ...
    expect(fetchFile).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        fetchUrl:
          'https://github.com/kozmoai/community/tree/main/glint-community-sessions/assets/Glint%20Community%20Sessions.png',
      }),
    );
  });
});
