...
import { makeCreatePermissionRule } from '@kozmoai/plugin-permission-node';
import {
...
import { z } from 'zod';
import { JsonObject, JsonPrimitive } from '@kozmoai/types';
...
export const hasTag = createTemplatePermissionRule({
  ...
  apply: (resource, { tag }) => {
    return resource['glint:permissions']?.tags?.includes(tag) ?? false;
  },
  toQuery: () => ({}),
});
