...
import { StructuredMetadataTable } from '@kozmoai/core-components';
      ...
      .map(([key, value]) => {
        for (const step of props.schemas) {
          ...
          if (definitionInSchema) {
            const glintReviewOptions =
              definitionInSchema['ui:glint']?.review;

            if (glintReviewOptions) {
              if (glintReviewOptions.mask) {
                return [key, glintReviewOptions.mask];
              }
              if (glintReviewOptions.show === false) {
                return [];
              }
            }
