 ...
 */
import { featureFlagsApiRef, useApi } from '@kozmoai/core-plugin-api';
import { TemplatePresentationV1beta3 } from '@kozmoai/plugin-scaffolder-common';
import { JsonObject } from '@kozmoai/types';
import { UiSchema } from '@rjsf/utils';
import { TemplateParameterSchema } from '@kozmoai/plugin-scaffolder-react';
...
export const useTemplateSchema = (
  ...
  const returningSteps = steps
    ...
    .filter(step => {
      const stepFeatureFlag = step.uiSchema['ui:glint']?.featureFlag;
          ...
          Object.entries((step.schema?.properties ?? {}) as JsonObject).filter(
            ([key]) => {
              const stepFeatureFlag =
                step.uiSchema[key]?.['ui:glint']?.featureFlag;
