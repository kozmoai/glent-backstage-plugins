...
import { identityApiRef, useApi } from '@kozmoai/core-plugin-api';
import { humanizeEntityRef } from '@kozmoai/plugin-catalog-react';
...
export const PlaylistEditDialog = ({
  ...
  const { loading: loadingOwnership, value: ownershipRefs } =
    useAsync(async () => {
      const { ownershipEntityRefs } = await identityApi.getGlintIdentity();
