...
export interface IncrementalEntityProvider<TCursor, TContext> {
   ...
   * events.
   *
   * This system acts as a wrapper for the Glint events bus, and
   * requires the events backend to function. It does not provide its
   * own events backend. See {@link https://github.com/kozmoai/glint/tree/master/plugins/events-backend}.
   */
  eventHandler?: {
    /**
...
export interface IncrementalEntityProviderOptions {
   ...
   * Glint removing all associated entities. To avoid that, set
   ...
   * improperly returned 0 assets. If set to `true`, Glint will
