...
```yaml
glint.io/linguist: https://url.to/your/srouce/code
```
...
Here's what that will look like in action using the Glint repo as an example:
...
metadata:
  # ...
  annotations:
    glint.io/linguist: https://github.com/kozmoai/glint
spec:
  type: service
  # ...
```
...
1. First we need to add the `@kozmoai/plugin-linguist` package to your frontend app:

   ```sh
   # From your Glint root directory
   yarn --cwd packages/app add @kozmoai/plugin-linguist
   ```
...
2. Second we need to add the `EntityLinguistCard` extension to the entity page in your app:
   ...
   import { isLinguistAvailable, EntityLinguistCard } from '@kozmoai/plugin-linguist';
