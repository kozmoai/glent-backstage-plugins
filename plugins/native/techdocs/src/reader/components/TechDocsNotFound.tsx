...
import { useApi, configApiRef, useAnalytics } from '@kozmoai/core-plugin-api';
import { ErrorPage } from '@kozmoai/core-components';
import { useTechDocsReaderPage } from '@kozmoai/plugin-techdocs-react';
...
export const TechDocsNotFound = ({ errorMessage }: Props) => {
  ...
  if (techdocsBuilder !== 'local') {
    additionalInfo =
      "Note that techdocs.builder is not set to 'local' in your config, which means this Glint app will not " +
      "generate docs if they are not found. Make sure the project's docs are generated and published by some external " +
      "process (e.g. CI/CD pipeline). Or change techdocs.builder to 'local' to generate docs from this Glint " +
      'instance.';
  }
