...
import { entityKindSchemaValidator } from '@kozmoai/catalog-model';
import type {
...
describe('templateEntityV1beta3Validator', () => {
  ...
  beforeEach(() => {
    entity = {
      ...
      spec: {
        ...
        parameters: {
          required: ['owner'],
          'glint:permissions': { tags: ['one', 'two'] },
          properties: {
            owner: {
              type: 'string',
              title: 'Owner',
  ...
  it('ignores unknown apiVersion', async () => {
    (entity as any).apiVersion = 'glint.io/v1beta0';
  ...
  it('rejects parameters with wrong glint:permissions', async () => {
  ...
  it('rejects steps with wrong glint:permissions', async () => {
    entity.spec.steps[0]['glint:permissions']!.tags = true as unknown as [];
    expect(() => validator(entity)).toThrow(/must be array/);

    entity.spec.steps[0]['glint:permissions'] = true as {};
    expect(() => validator(entity)).toThrow(/must be object/);
  });
});
