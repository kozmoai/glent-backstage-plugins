...
This is the backend for the default Glint Tech Insights feature.
...
yarn --cwd packages/backend add @kozmoai/plugin-tech-insights-backend
```
...
You can use the extension points [@kozmoai/plugin-tech-insights-node](../tech-insights-node)
...
To create factRetrieverRegistration you need to implement `FactRetriever` interface defined in `@kozmoai/plugin-tech-insights-node` package (see [Creating fact retrievers](#creating-fact-retrievers) for details). After you have implemented this interface you can wrap that into a registration object like follows:

```ts
import { createFactRetrieverRegistration } from '@kozmoai/plugin-tech-insights-backend';
...
The Tech Insights plugin utilizes the `PluginTaskScheduler` for scheduling tasks and coordinating the task invocation across instances. See [the PluginTaskScheduler documentation](https://glint.io/docs/reference/backend-tasks.plugintaskscheduler) for more information.
...
There is a default FactChecker implementation provided in module `@kozmoai/plugin-tech-insights-backend-module-jsonfc`. This implementation uses `json-rules-engine` as the underlying functionality to run checks. If you want to implement your own FactChecker, for example to be able to handle other than `boolean` result types, you can do so by implementing `FactCheckerFactory` and `FactChecker` interfaces from `@kozmoai/plugin-tech-insights-common` package.
        ...
        },
        {
          ...
          rule: {
            conditions: {
              all: [
                {
                  fact: 'hasAnnotationGlintIoTechdocsRef',
                  operator: 'equal',
                  value: true,
                },
              ],
