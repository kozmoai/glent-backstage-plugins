...
- **Simple (Recommended)** - This gives you an out-of-the-box Tech Radar experience. It lives on the `/tech-radar` URL of your Glint installation.
...
yarn --cwd packages/app add @kozmoai/plugin-tech-radar
```
...
import { TechRadarPage } from '@kozmoai/plugin-tech-radar';
...
The `TechRadar` plugin uses the `techRadarApiRef` to get a client which implements the `TechRadarApi` interface. The default sample one is located [here](https://github.com/kozmoai/glint/blob/master/plugins/tech-radar/src/sample.ts). To load your own data, you'll need to provide a class that implements the `TechRadarApi` and override the `techRadarApiRef` in the `app/src/apis.ts`.
...
} from '@kozmoai/plugin-tech-radar';
...
import { techRadarApiRef } from '@kozmoai/plugin-tech-radar';
...
You can use the `svgProps` option to pass custom React props to the `<svg>` element we create for the Tech Radar. This complements well with the `data-testid` attribute and the `@testing-library/react` library we use in Glint.
