...
import { EntityFilterQuery } from '@kozmoai/catalog-client';
...
/**
 * A representation of a successful Glint sign-in.
 *
 * Compared to the {@link GlintIdentityResponse} this type omits
 * the decoded identity information embedded in the token.
 *
 * @public
 */
export interface GlintSignInResult {
  /**
   * The token used to authenticate the user within Glint.
   */
  token: string;
}

/**
 * Response object containing the {@link GlintUserIdentity} and the token
 * from the authentication provider.
 *
 * @public
 */
export interface GlintIdentityResponse extends GlintSignInResult {
  /**
   ...
   */
  identity: GlintUserIdentity;
}

/**
 * User identity information within Glint.
 *
 * @public
 */
export type GlintUserIdentity = {
  /**
...
/**
 * Parameters used to issue new Glint Tokens
 *
 * @public
 */
export type TokenParams = {
   ...
   * `iat`, and `exp`. The Glint team also maintains the right add new claims in the future
...
export type AuthResolverContext = {
  /**
   * Issues a Glint token using the provided parameters.
   */
...
export type ClientAuthResponse<TProviderInfo> = {
  ...
  profile: ProfileInfo;
  glintIdentity?: GlintIdentityResponse;
};
 ...
 * authentication. Must return a valid {@link @kozmoai/plugin-auth-node#GlintSignInResult}.
 *
 * @public
 */
 ...
 *
 * It is also temporarily used as the profile of the signed-in user's Glint
