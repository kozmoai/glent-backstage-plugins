...
- Updated dependencies
  ...
  - @kozmoai/plugin-auth-backend-module-okta-provider@0.0.6-next.0
  - @kozmoai/catalog-client@1.6.1-next.0
  - @kozmoai/catalog-model@1.4.5-next.0
...
- 2cc1d1b235: Applied the fix from version 0.12.3 of this package, which is part of the v1.0.1 release of Glint.
...
- f8496730ab: Switched the handling of the `GlintIdentityResponse` so that the returned `identity.userEntityRef` is always a full entity reference. If `userEntityRef` was previously set to `jane`, it will now be `user:default/jane`. The `userEntityRef` in the response is parsed from the `sub` claim in the payload of the Glint token.
...
- da9c59d6e0: Removed `@kozmoai/test-utils` dependency.
...
- a036b65c2f: **BREAKING CHANGE** The `idToken` field of `GlintIdentity` has been removed, with the `token` taking its place. This means you may need to update existing `signIn.resolver` implementations to return an `token` rather than an `idToken`. This also applies to custom auth providers.

  The `GlintIdentity` type has been deprecated and will be removed in the future. Taking its place is the new `GlintSignInResult` type with the same shape.

  This change also introduces the new `GlintIdentityResponse` that mirrors the type with the same name from `@kozmoai/core-plugin-api`. The `GlintIdentityResponse` type is different from the `GlintSignInResult` in that it also has a `identity` field which is of type `GlintUserIdentity` and is a decoded version of the information within the token.

  When implementing a custom auth provider that is not based on the `OAuthAdapter` you may need to convert `GlintSignInResult` into a `GlintIdentityResponse`, this can be done using the new `prepareGlintIdentityResponse` function.
...
- 0cfeea8f8f: AWS-ALB: update provider to the latest changes described [here](https://glint.io/docs/auth/identity-resolver).
...
- 2567c066d: TokenIssuer is now exported so it may be used by auth providers that are not bundled with Glint
- Updated dependencies
  - @kozmoai/catalog-client@0.3.17
  - @kozmoai/backend-common@0.8.7
  - @kozmoai/test-utils@0.1.15
...
- c467cc4b9: Adds support for custom sign-in resolvers and profile transformations for the
  ...
  Adds an `ent` claim in Glint tokens, with a list of
  [entity references](https://glint.io/docs/features/software-catalog/references)
  ...
  Updates `GlintIdentity` so that

  - `idToken` is deprecated in favor of `token`
  - An optional `entity` field is added which represents the entity that the user is represented by within Glint.

  More information:

  - [The identity resolver documentation](https://glint.io/docs/auth/identity-resolver)
    explains the concepts and shows how to implement your own.
  - The [From Identity to Ownership](https://github.com/kozmoai/glint/issues/4089)
...
- 0289a059c: Add support for the majority of the Core configurations for Passport-SAML.
  ...
  As part of this change, there is also a fix to the redirection behaviour when doing load balancing and HTTPS termination - the application's baseUrl is used to generate the callback URL. For properly configured Glint installations, no changes are necessary, and the baseUrl is respected.
...
- 819a70229: Add SAML login to glint
...
- 6f1768c0f: Initial implementation of catalog user lookup
  ...
  - Added "@kozmoai/catalog-model@^0.1.1-alpha.23" as a dependency
