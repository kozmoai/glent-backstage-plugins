...
import { AuthResolverContext } from '@kozmoai/plugin-auth-node';
...
const glintIdentityTokenClaims = {
...
describe('EasyAuthAuthProvider', () => {
  ...
  async function signInResolver() {
    return {
      id: 'user.name',
      token: await buildJwt(glintIdentityTokenClaims),
    };
  }
...
describe('easyAuth factory', () => {
  ...
  it('should fail when run outside of Azure App Services', async () => {
    ...
    expect(() => factory({} as any)).toThrow(
      'Glint is not running on Azure App Services',
    );
  });
