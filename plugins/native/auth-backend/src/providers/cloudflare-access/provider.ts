...
import { prepareGlintIdentityResponse } from '../prepareGlintIdentityResponse';
...
export class CloudflareAccessAuthProvider implements AuthProviderRouteHandlers {
  ...
  async refresh(req: express.Request, res: express.Response): Promise<void> {
    // ProxiedSignInPage calls `/refresh` implicitly each time the glint
  ...
  private async handleResult(
    ...
    const glintIdentity = await this.signInResolver(
      {
        result,
        profile,
      },
    ...
    return {
      ...
      glintIdentity: prepareGlintIdentityResponse(glintIdentity),
      profile,
    };
  }
}
...
export const cfAccess = createAuthProviderIntegration({
  create(options: {
    ...
    signIn: {
      /**
       * Maps an auth result to a Glint identity for the user.
       */
      resolver: SignInResolver<CloudflareAccessResult>;
    };
    /**
     * CacheClient object that was configured for the Glint backend,
