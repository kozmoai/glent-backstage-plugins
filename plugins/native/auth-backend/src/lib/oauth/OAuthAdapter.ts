...
import { prepareGlintIdentityResponse } from '../../providers/prepareGlintIdentityResponse';
...
/**
 * @public
 * @deprecated Use `createOAuthRouteHandlers` from `@kozmoai/plugin-auth-node` instead
 */
...
export class OAuthAdapter implements AuthProviderRouteHandlers {
  ...
  async frameHandler(
    ...
    try {
      ...
      const identity = await this.populateIdentity(response.glintIdentity);

      const responseObj: WebMessageResponse = {
        type: 'authorization_response',
        response: { ...response, glintIdentity: identity },
      };
  ...
  async refresh(req: express.Request, res: express.Response): Promise<void> {
    ...
    try {
      ...
      const glintIdentity = await this.populateIdentity(
        response.glintIdentity,
      );
      ...
      res.status(200).json({ ...response, glintIdentity });
    } catch (error) {
  ...
  /**
   * If the response from the OAuth provider includes a Glint identity, we
   * make sure it's populated with all the information we can derive from the user ID.
   */
  private async populateIdentity(
    identity?: GlintSignInResult,
    ...
    return prepareGlintIdentityResponse(identity);
  }
