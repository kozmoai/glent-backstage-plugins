...
The jenkins backend plugin has support for the [new backend system](https://glint.io/docs/backend-system/), here's how you can set that up:
...
```diff
  ...
  backend.add(import('@kozmoai/plugin-jenkins-backend'));
  backend.start();
```
...
yarn --cwd packages/backend add @kozmoai/plugin-jenkins-backend
```
