...
- b2e6cb6acf: **Breaking**
  ...
  ```ts
  // catalog.ts
  import { GithubLocationAnalyzer } from '@kozmoai/plugin-catalog-backend-module-github';
  ...
...
- 6a6ec777ce: Switched to using the `ScmAuthApi` for authentication rather than GitHub auth. If you are instantiating your `CatalogImportClient` manually you now need to pass in an instance of `ScmAuthApi` instead.
  ...
  Also be sure to register the `scmAuthApiRef` from the `@kozmoai/integration-react` in your app:

  ```ts
  import { ScmAuth } from '@kozmoai/integration-react';
    ...
    <Page themeId="home">
      ...
      <Content>
        <ContentHeader title="Start tracking your components">
          <SupportButton>
            Start tracking your component in Glint by adding it to the
            software catalog.
          </SupportButton>
        </ContentHeader>
  ...
  <Route
    ...
    element={
      <CatalogImportPage
        pullRequest={{
          preparePullRequest: () => ({
            title: 'chore: add glint catalog file [skip ci]',
          }),
        }}
      />
    }
...
- 68dd79d83: The plugin has been refactored and is now based on a configurable state machine of 'analyze', 'prepare', 'review' & 'finish'.
  ...
  During the refactoring, the `catalogRouteRef` property of the `CatalogImportPage` has been removed, so the `App.tsx` of the glint apps need to be updated:
...
- be5ac7fde: Remove dependency to `@kozmoai/plugin-catalog-backend`.
...
- Updated dependencies [071711d70]
  - @kozmoai/plugin-catalog-backend@0.5.2
...
- Updated dependencies [0097057ed]
  - @kozmoai/plugin-catalog-backend@0.5.0
  - @kozmoai/catalog-model@0.6.0
  - @kozmoai/core@0.4.1
  - @kozmoai/integration@0.1.4
...
- Updated dependencies [ff243ce96]
  - @kozmoai/plugin-catalog-backend@0.4.0
  - @kozmoai/core@0.4.0
...
- Updated dependencies [ebf37bbae]
  - @kozmoai/plugin-catalog-backend@0.3.0
