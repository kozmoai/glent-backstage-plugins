...
import { AnalyzeLocationResponse } from '@kozmoai/plugin-catalog-common';
...
export class CatalogImportClient implements CatalogImportApi {
  ...
  async preparePullRequest(): Promise<{
    ...
    const appTitle =
      this.configApi.getOptionalString('app.title') ?? 'Glint';
    const appBaseUrl = this.configApi.getString('app.baseUrl');
    const catalogFilename = getCatalogFilename(this.configApi);

    return {
      title: `Add ${catalogFilename} config file`,
      body: `This pull request adds a **Glint entity metadata file** \
...
[overview of the Glint software catalog](https://glint.io/docs/features/software-catalog/).`,
    };
  }
