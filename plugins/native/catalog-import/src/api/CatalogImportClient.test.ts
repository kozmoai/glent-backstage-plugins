...
import { ConfigReader, UrlPatternDiscovery } from '@kozmoai/core-app-api';
import { ScmIntegrations } from '@kozmoai/integration';
import { ScmAuthApi } from '@kozmoai/integration-react';
import { CatalogApi } from '@kozmoai/plugin-catalog-react';
import { setupRequestMockHandlers } from '@kozmoai/test-utils';
...
describe('CatalogImportClient', () => {
  ...
  const mockBaseUrl = 'http://glint:9191/api/catalog';
  ...
  describe('analyzeUrl', () => {
    ...
    it('should reject for integrations that are not github ones', async () => {
      await expect(
        catalogImportClient.analyzeUrl(
          'https://registered-but-not-github.com/kozmoai/glint',
        ),
      ).rejects.toThrow(
        new Error(
    ...
    it('should find locations from github', async () => {
              ...
              existingEntityFiles: [
                {
                  ...
                  location: {
                    type: 'url',
                    target:
                      'https://github.com/kozmoai/glint/blob/main/simple/path/catalog-info.yaml',
                  },
                  entity: {
                    apiVersion: '1',
                    kind: 'k',
                ...
                },
                {
                  ...
                  location: {
                    type: 'url',
                    target:
                      'https://github.com/kozmoai/glint/blob/main/co/mple/x/path/catalog-info.yaml',
                  },
                  entity: {
                    apiVersion: '1',
                    kind: 'k',
                ...
                },
                {
                  ...
                  location: {
                    type: 'url',
                    target:
                      'https://github.com/kozmoai/glint/blob/main/catalog-info.yaml',
                  },
                  entity: {
                    apiVersion: '1',
                    kind: 'k',
      ...
      await expect(
        catalogImportClient.analyzeUrl(
          'https://github.com/kozmoai/glint',
        ),
      ).resolves.toEqual({
        locations: [
          {
    ...
    it('should find repository from github', async () => {
      ...
      await expect(
        ...
        type: 'repository',
        url: 'https://github.com/kozmoai/glint',
  ...
  describe('submitPullRequest', () => {
    it('should create GitHub pull request', async () => {
      ...
      await expect(
        catalogImportClient.submitPullRequest({
          repositoryUrl: 'https://github.com/kozmoai/glint',
          fileContent: `
            {
                ...
                "metadata": {
                  "name": "valid-name",
                  "annotations": {
                      "github.com/project-slug": "glint/example-repo"
                }
              },
              "spec": {
                  "type": "other",
      ...
      expect(
        ...
        owner: 'glint',
        repo: 'glint',
        ref: 'refs/heads/glint-integration',
        sha: 'any',
      });
      expect(
    ...
    it('Submit Pull Request with invalid component name', async () => {
      const ErrorMessage =
        'Policy check failed for component:default/invalid name; caused by Error: "metadata.name" is not valid; expected a string that is sequences of [a-zA-Z0-9] separated by any of [-_.], at most 63 characters in total but found "invalid name". To learn more about catalog file format, visit: https://github.com/kozmoai/glint/blob/master/docs/architecture-decisions/adr002-default-catalog-file-format.md';
