...
- Import `catalog-info.yaml` files from a URL in a repository of one of the supported Git integrations (example `https://github.com/kozmoai/glint/catalog-info.yaml`).
- _[GitHub only]_ Search for all `catalog-info.yaml` files in a Git repository (example: `https://github.com/kozmoai/glint`).
...
yarn --cwd packages/app add @kozmoai/plugin-catalog-import
```
...
import { CatalogImportPage } from '@kozmoai/plugin-catalog-import';
...
Entity filename (default: `catalog-info.yaml`) and branch name (default: `glint-integration`) used in pull requests can be configured in `app-config.yaml` as follows:
