...
import { GLINTApi, glintApiRef } from '../../api';
...
const GlintAppGeneralInfo = ({
  input: { gitHostUrl, gitApp, repoSecretArn, api, appPending }
}: { input: { account: string, region: string, gitHostUrl: string, gitApp: string, repoSecretArn: string, api: GLINTApi, appPending: boolean } }) => {
  ...
  async function getData() {
    ...
    const gitHostParam = await api.getPlatformSSMParam({ paramName: "/glint/gitlab-hostname" });
...
export const GeneralInfoCard = ({ appPending }: { appPending: boolean }) => {
  ...
  if (appPending) {
    ...
    return <GlintAppGeneralInfo input={input} />;
  }
  else {
