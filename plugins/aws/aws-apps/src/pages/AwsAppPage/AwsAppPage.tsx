...
import { GenericAWSEnvironment, readGlintAppAuditPermission } from '@aws/plugin-aws-apps-common-for-backstage';
...
import { isGitlabAvailable } from '@kozmoai/backstage-plugin-gitlab';
...
export function AwsAppPage(_props: AwsAppPageProps) {
  ...
  const { loading: loadingPermission, allowed: canReadAudit } = usePermission({
    permission: readGlintAppAuditPermission,
  });
      ...
      <EntityLayout>
        ...
        {!loadingPermission && canReadAudit && (
          <EntityLayout.Route path="/audit" title="Audit">
            <RequirePermission permission={readGlintAppAuditPermission} errorPage={<></>}>
              {auditContent}
            </RequirePermission>
          </EntityLayout.Route>
        )}
